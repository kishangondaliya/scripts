...
def create_annotations(dbpath, subset, dst):
    ...
    annotations_path = os.path.join(os.path.abspath(dbpath), 'annotations/instances_{}2014.json'.format(subset))
    #images_path = path(dbpath).expand() / 'images/{}2014'.format(subset)
    images_path = os.path.join(os.path.abspath(dbpath), 'images/{}2014'.format(subset))
